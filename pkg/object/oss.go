...
func (o *ossClient) SetCheckSumKey(meta string) error {
	o.checkSumKey = meta
	return nil
}

func (o *ossClient) IsSetMd5(flag bool) error {
	if flag {
		o.sumAlgorithm = checksumMd5
	}
	return nil
}
...
func (o *ossClient) checkError(err error) error {
	...
	if strings.Contains(msg, "InvalidAccessKeyId") || strings.Contains(msg, "SecurityTokenExpired") {
		logger.Error().Msgf("refresh security token: %s", err)
		go o.refreshToken()
	}
	return err
}
	...
	if off > 0 || limit > 0 {
		...
		if err == nil {
			resp = verifyChecksum(resp,
				resp.(*oss.Response).Headers.Get(o.checkSumKey))
		}
	}
...
func (o *ossClient) Put(key string, in io.Reader, acl models.CannedACLType) error {
	checkSumMetaKey := oss.HTTPHeaderOssMetaPrefix + o.sumAlgorithm.String()
	if ins, ok := in.(io.ReadSeeker); ok {
		option := oss.Meta(checkSumMetaKey, generateChecksum(ins, o.sumAlgorithm))
		return o.checkError(o.bucket.PutObject(key, in, option))
	}
	var ossAcl oss.Option
	if acl != "" && acl != models.AuthenticatedRead {
		switch acl {
		case models.Default, "":
			ossAcl = oss.ACL(oss.ACLDefault)
		case models.Private:
			ossAcl = oss.ACL(oss.ACLPrivate)
		case models.PublicRead:
			ossAcl = oss.ACL(oss.ACLPublicRead)
		case models.PublicReadWrite:
			ossAcl = oss.ACL(oss.ACLPublicReadWrite)
		}
	}
	return o.checkError(o.bucket.PutObject(key, in, ossAcl))
}
...
func (o *ossClient) CreateMultipartUpload(key string, minSize int, acl models.CannedACLType) (*MultipartUpload, error) {
	var ossAcl oss.Option
	if acl != "" && acl != models.AuthenticatedRead {
		switch acl {
		case models.Default, "":
	...
	r, err := o.bucket.InitiateMultipartUpload(key, ossAcl)
	if o.checkError(err) != nil {
		return nil, err
	}
	return &MultipartUpload{UploadID: r.UploadID, MinPartSize: minSize, MaxCount: 10000}, nil
}
...
func (o *ossClient) GetObjectAcl(key string) (models.CannedACLType, error) {
	acl, err := o.bucket.GetObjectACL(key)
	if err != nil {
		return "", err
	}
	switch acl.ACL {
...
func (o *ossClient) refreshToken() time.Time {
	...
	if err != nil {
		logger.Error().Msgf("refresh token: %s", err)
	...
	logger.Debug().Msgf("Refreshed STS, will be expired at %s", cred.Expiration)
	expire, err := time.Parse("2006-01-02T15:04:05Z", cred.Expiration)
	if err != nil {
		logger.Error().Msgf("invalid expiration: %s, %s", cred.Expiration, err)
...
func newOSS(endpoint, accessKey, secretKey string) (ObjectStorage, error) {
	...
	if domain == "" {
		...
		logger.Debug().Msgf("Use endpoint %q", domain)
	}
	...
	o := &ossClient{client: client, bucket: bucket, checkSumKey: oss.HTTPHeaderOssMetaPrefix + checksumCrc32.String(), sumAlgorithm: checksumCrc32}
	if securityToken != "" {
		go func() {
			for {
				next := o.refreshToken()
